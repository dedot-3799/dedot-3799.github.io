//メイン処理用スレッドとサブ処理スレッドの管理
class mainWorker {
    constructor() {
        this.worker = new Worker("./mainWorker.js");
        this.multiThreadManager = new multiThreadManager(parseInt(navigator.hardwareConcurrency || 4));
        this.callbacks = new Map();

        this.worker.addEventListener("message", (event) => {
            const { delegate, uuid, time, result, type, threads } = event.data;
            if (delegate) {
                // マルチスレッドマネージャーにメッセージを送信して結果を処理スレッドに返す
                this.multiThreadManager.request({ delegate, uuid, time, result, type, threads }, threads).then((r) => { this.worker.postMessage({ resultFromMLTI: true, uuid, type, time, result: r }) });
            } else if (this.callbacks.has(uuid)) {
                // そのままresolve
                this.callbacks.get(uuid)(result);
                this.callbacks.delete(uuid);
            }
            if (time == -255) { throw new Error("Worker Error") } // ワーカー内で致命的なエラーが発生した場合に例外を投げる
        });
    ...
    //　数式評価(有理数)
    evaluateFormulaQ(formula) {
        return this.request("evaluateFormulaQ", formula);
    }

    //　数式評価(実数)
    evaluateFormulaR(formula) {
        return this.request("evaluateFormulaR", formula);
    }
    ...
    // サブスレッド管理
    manageSubThreads(option) {
        switch (option.action) {
            case "add":
                this.multiThreadManager.addThread(option.count);
                break;
            case "remove":
                this.multiThreadManager.removeThread(option.count);
                break;
            case "resize":
                this.multiThreadManager.resizeThread(option.count);
                break;
            case "restart":
                this.multiThreadManager.restartAllThreads();
                break;
            default:
...
class multiThreadManager {
    constructor(count = 4) {
        ...
        this.isBusy = [];
        this.queue = [];
        this.queueBusyThread = new Map();
        this.requestResize = 4;
        this.callbacks = new Map();
        this.results = new Map();
        this.threadCount = new Map();
        this.subThreadSrc = "./subProcessingThread.js";

        // ワーカープールの初期化
        for (let i = 0; i < count; i++) {
            const worker = new Worker(this.subThreadSrc);
            worker.onmessage = e => this.onMessage(e, worker);
            worker.onerror = e => this.restartThreads(worker);
            this.isBusy.push(false);
    ...
    onMessage(event) {
        ...
        this.results.get(uuid).push(result);
        console.log(uuid, event.data.threadId)
        if (this.callbacks.has(uuid) && this.results.get(uuid).length === this.threadCount.get(uuid)) {
            console.log(`multiThreadManager: All results received for UUID: ${uuid}. Resolving promise. (${this.results.get(uuid).length})`);
            let result = this.results.get(uuid);
            this.callbacks.get(uuid)(result);
            this.callbacks.delete(uuid);
            this.results.delete(uuid);
            this.threadCount.delete(uuid);
        }
        this.isBusy[event.data.threadId] = false;
        if (!this.queue.length && this.isBusy.every(busy => !busy) && this.requestResize) {
            this.resizeThread(this.requestResize);
        }
    }

    // リクエストをキューに追加する共通関数
    request(opt, threadCount = this.pool.length) {
        return new Promise((resolve) => {
            this.callbacks.set(opt.uuid, resolve);
            this.results.set(opt.uuid, []);
            this.threadCount.set(opt.uuid, threadCount);
            for (let i = 0; i < threadCount; i++) {
                this.queue.push({ option: opt, threadId: i, totalThreads: threadCount });
            }
            this.processQueue(threadCount);
            this.requestResize = threadCount;
        });
    }

    // キューに投げた仕事を各スレッドに割り当てて処理する
    processQueue(threadCount = this.pool.length) {
        let totalThreads = threadCount;
        let recentExecutableThread = this.isBusy.filter(v => !v).length;
        console.log(recentExecutableThread)
        if (recentExecutableThread < totalThreads) this.addThread(totalThreads - recentExecutableThread);
        let threadsAssigned = 0;
        for (let threadId = 0; threadsAssigned < totalThreads; threadId++) {
            if (this.isBusy[threadId] || !this.pool[threadId]) {
                continue;
            }
            const thread = this.pool[threadId];
            const jobData = this.queue.shift();
            const job = jobData.option;
            const threadsID = jobData.threadId;
            this.isBusy[threadId] = true;
            thread.postMessage({ job: job, THID: threadId, threadId: threadsID, totalThreads: totalThreads });
            threadsAssigned++;
        }
    }

    // スレッド追加
    addThread(addCount = 1) {
    ...
    // スレッド数変更
    resizeThread(newCount) {
        if (newCount > this.pool.length) {
            this.addThread(newCount - this.pool.length);
        } else {
            this.removeThread(this.pool.length - newCount);
        }
    }
    ...
    restartAllThreads() {
        for (let worker of this.pool) {
            worker.terminate();
            const newWorker = new Worker(this.subThreadSrc);
            newWorker.onmessage = e => this.onMessage(e, newWorker);
    ...
    // スレッドIDを指定して再起動
    restartThreads(threadId = null) {
        if (threadId === null) {
            this.restartAllThreads();
        } else {
            const worker = this.pool[threadId];
            if (worker) {
                worker.terminate();
                const newWorker = new Worker(this.subThreadSrc);
                newWorker.onmessage = e => this.onMessage(e, newWorker);
                this.pool[threadId] = newWorker;
                this.isBusy[threadId] = false;
            }
        }
    }
}
...
async function processCommand(command) {
    ...
    try {
        if ((mainCommand === 'prime' || mainCommand === "p" || mainCommand === "prim" || mainCommand === '/prime' || mainCommand === "/p" || mainCommand === "/prim") && parts.length === 2) {
            ...
            let et = performance.now();
            document.getElementById('cli-box').innerHTML += `<p>${msg}(${Math.round((et * 10) - (st * 10)) / 10}ms)</p>`;
        } else if ((mainCommand === 'calculate' || mainCommand === "c" || mainCommand === "calc" || mainCommand === '/calc' || mainCommand === "/c" || mainCommand === "/calc") && parts.length < 3) {
            let str;
            if (parts.length < 2) {
                throw new Error(`Invalid input. Please enter a expression.`);
            }
            const str2 = parts[1].split("");
            for (let i = 0; i < str2.length; i++) {
                if (isNaN(parseInt(str2[i])) && !str2[i].match(/[\^\*×\/÷\-\+\(\)\!\ ]/) && isNaN(parseInt(toHankakuNumber(str2[i])))) {
            ...
            str = await main.evaluateFormula(parts[1]);
            let et = performance.now();
            const msg = `${parts[1]} = ${str}.`;
            document.getElementById('cli-box').innerHTML += `<p>${msg}(${Math.round((et * 10) - (st * 10)) / 10}ms)</p>`;
        } else if ((mainCommand === 'calculate' || mainCommand === "c" || mainCommand === "calc" || mainCommand === '/calculate' || mainCommand === '/calc' || mainCommand === "/c" || mainCommand === "/calc") && (parts[1] == "Q" || parts[1] == "q" || parts[1] == "Quotient") && parts.length < 4) {
            let str;
            if (parts.length < 3) {
                throw new Error(`Invalid input. Please enter a expression.`);
            }
            ...
            await wait(1);
            let st = performance.now();
            str = await main.evaluateFormulaQ(parts[2]);
            let et = performance.now();
            const msg = `${parts[2]} = ${str}.`;
            document.getElementById('cli-box').innerHTML += `<p>${msg}(${Math.round((et * 10) - (st * 10)) / 10}ms)</p>`;
            ...
            console.log(str);
            await wait(1);
            if (BigInt(str) === Infinity) {
                throw new Error(`An overflow has occurred during the calculation process of the system. The range of numbers that can be processed is less than or equal to 2^53 = 9007199254740992. Please enter a smaller number.`);
            };
            await wait(1);
            let st = performance.now();
            const factors = await main.findNearPrimeMLTI({ number: BigInt(str), opt: "ud" });
            let msg = `An example of a prime number close to ${str !== parts[2] ? parts[2] + " = " : ""}${str} is ${factors}.`;
            if (String(factors).match(/Error/)) {
...
async function checkPrime() {
    ...
    try {
        ...
        let et = performance.now();
        if (String(r).match(/Error/)) {
            throw new Error(r);
        }
        await wait(1);
        for (let g = 0; g < 2; g++) {
            resultDiv.innerHTML = r === true ? `<p>${String(number).length}桁の自然数 ${number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} は</p><p class="nmlrslt">素数です。</p><p class="smallF">数式解析:${Math.round((pet * 10) - (pst * 10)) / 10}ms, 判定:${Math.round((et * 10) - (st * 10)) / 10}ms</p>` : `<p>${String(number).startsWith("-") ? String(number).length - 1 : String(number).length}桁の整数 ${number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} は</p><p class="nmlrslt">素数ではありません。</p><p class="smallF">数式解析:${Math.round((pet * 10) - (pst * 10)) / 10}ms, 判定:${Math.round((et * 10) - (st * 10)) / 10}ms</p>`;
            await wait(100);
            await repaint();
        }
    } catch (error) {
...
async function handleCommand() {
    ...
    if (commandParts[0] === '/prime' && commandParts.length === 2) {
    ...
    } else if (commandParts[0].startsWith('/')) {
        appendMessage(`${commandParts[0]}`, 'user', userName, 'user_icon.png', 'user');
        appendTypingIndicator();
        setTimeout(() => {
            removeTypingIndicator();
            appendMessage(`Error: Invalid command '${commandParts[0]}'.`, 'bot', "Primality Tester Command Processor", 'prime.png', '');
        }, 1000);
    } else {
        // 対話形式の処理
        await wait(0);
        ...
        setTimeout(async () => {
            removeTypingIndicator();
            if (grl !== false && String(grl).length !== 0 && typeof grl === "string") {
