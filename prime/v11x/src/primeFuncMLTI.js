async function aboutPrimeMLTI(num, option) {
    ...
    switch (option) {
        case "up":
            return await aboutPrime(n, "up");
        case "dn":
            return await aboutPrime(n, "dn");
        case "ud": {
...
async function apr_cl_f(N, uuid, threadNumber) {
    console.log(uuid);
    ...
    if (String(N).length >= 25) {
        ...
        let hje = await SPTM.sendMaeeageToSubThread({ delegate: true, type: "APR-CL-MLTI", uuid: uuid, result: { l: l, q_list: q_list, et: et, nql2: nq_l2, num: N} ,threads: threadNumber },uuid)
...
function apr_cl_l(results) {
    let { l, et, num } = results[0].rs;
    const N = num;
    for (let bk = 0; bk < results.length; bk++) {
        const x = results[bk].result;
        if (!x) {
            return false;
        }
        let xd = x.split(",");
