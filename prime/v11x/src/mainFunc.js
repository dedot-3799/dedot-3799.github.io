...
async function isPrime(n,uuid) {
    ...
    if (nB > 9007199254740992n) {
        if (((nB + 1n) & nB) == 0) {
            ...
            } else if (methodN == "APR-CL-MULTI" && uuid) {
                return apr_cl_f(nB,uuid,Number(settingsManager("get","main.threads")));
            } else {
                return APRtest(nB);
            }
        }
    ...
    switch (methodN) {
        ...
        case "APR-CL-MULTI": {
            result = await apr_cl_f(nB,uuid,Number(settingsManager("get","main.threads")));
            if (result == "MLTI-T") return;
            break;
        }
        case "MILLER-RABIN-64": {
