class subWorkerRequestManager {
    ...
    // リクエストを送る共通関数
    request(type, option, uuid = crypto.randomUUID()) {
        return new Promise((resolve) => {
            this.callbacks.set(uuid, resolve);
            for (let i = 0; i < this.workers.size; i++) {
                this.workers.get(i).request(type, option, i , uuid);
            }
        });
    }

    // ワーカー作成
    createWoker(numberOfWorkers) {
        for (let i = 0; i < numberOfWorkers; i++) {
            this.workers.set(i, new Worker("./subProcessingSystem.js"));
        }
    }

    // ワーカーの数を変更する関数
    refleshNumberOfWorkers(numberOfWorkers) {
        let prslen = numberOfWorkers - this.workers.size;
        for (let i = 0; i < Math.abs(prslen); i++) {
            if (prslen < 0) {
                this.workers.get(this.workers.size - i - 1).terminate();
                this.workers.delete(this.workers.size - i - 1);
            } else if (prslen > 0) {
                this.workers.set(this.workers.size + i, new Worker("./subProcessingSystem.js"));
            }
        }
    }
...
function settingsManager(mode, prf, set, opt) {
    if (mode === "set") {
        ...
        // preference GUIの更新
        if (prf !== "main.threads") {
        let optionsD = document.getElementById(prf).options;
        for (let i = 0; i < optionsD.length; i++) {
            if (optionsD[i].value == set) {
...
async function processCommand(command) {
    ...
    try {
        if ((mainCommand === 'prime' || mainCommand === "p" || mainCommand === "prim" || mainCommand === '/prime' || mainCommand === "/p" || mainCommand === "/prim") && parts.length === 2) {
            ...
            if (str.length == 0) throw new Error(`Invalid input. Please enter a natural number.`);
            const arg = parts[1];
            await wait(1);
            ...
            await wait(1);
            let st = performance.now();
            const factors = await main.primeFactorization(parseInt(parts[2]));
            let et = performance.now();
            const msg = factors === false ? `${parts[2]}${parseInt(parts[2]) < 2 ? "" : " is a prime number and"} cannnot be prime factorised.` : `${parts[2]} = ${factors} .`;
            ...
            if (parts[1] == "chat.cN" || parts[1] == "chat.chName" || parts[1] == "character" || parts[1] === "chat.character") {
                if (parts[2] == "set") {
                    settingsManager("set", "chat.character", parts[3])
                    document.getElementById('cli-box').innerHTML += `<p>Success.</p>`;
                } else if (parts[2] == "default" || parts[2] == "auto") {
                    settingsManager("set", "chat.character", "default");
                ...
                settingsManager("set", parts[1], parts[3]);
                document.getElementById('cli-box').innerHTML += `<p>Success.</p>`;
            }
        } else {
...
async function handleCommand() {
    ...
    if (commandParts[0] === '/prime' && commandParts.length === 2) {
        const num = parseInt(removeString(commandParts[1]), 10);
        appendMessage(`${removeString(commandParts[1])}は素数ですか？`, 'user', userName, 'user_icon.png', 'user');
        if (isNaN(num)) {
            ...
            document.getElementById('user-input').value = '';
            await wait(0);
            const isPrimeResult = await main.isPrime(BigInt(removeString(commandParts[1])));

            setTimeout(() => {
                removeTypingIndicator();
                const result = isPrimeResult ? `${removeString(commandParts[1]).length}桁の自然数${removeString(commandParts[1])}は**素数${randomDS("desu")}**` : `${removeString(commandParts[1]).length}桁の自然数${removeString(commandParts[1])}は**素数${randomDS("dewaNai")}**`;
        ...
        appendMessage(`${removeString(commandParts[2])}を素因数分解してください。`, 'user', userName, 'user_icon.png', 'user');
        if (BigInt(commandParts[2]) > 9007199254740992n) {
            appendTypingIndicator();
            setTimeout(() => {
                removeTypingIndicator();
            ...
            const factors = await main.primeFactorization(BigInt(commandParts[2]));
            const msg = factors === false ? `${commandParts[2]}は${parseInt(commandParts[2]) < 2 ? "" : "**素数なので**"}素因数分解できません。` : `$ ${commandParts[2]} =$$ ${factors} $ です。`;
            appendTypingIndicator();
            setTimeout(() => {
                removeTypingIndicator();
        ...
        appendMessage(`${removeString(commandParts[2])}に近い素数を挙げてください。`, 'user', userName, 'user_icon.png', 'user');
        if (commandParts[2].length == 0) {
            appendTypingIndicator();
            setTimeout(() => {
                removeTypingIndicator();
            ...
            let re = await main.findNearPrime({ number: BigInt(removeString(commandParts[2])), opt: "ud" })
            const msg = `${removeString(commandParts[2]).length}桁の自然数 ${removeString(commandParts[2])} に最も近い素数は** ${re} **${randomDS("desu")}`;
            appendTypingIndicator();
            setTimeout(() => {
                removeTypingIndicator();
        ...
        appendMessage(`${removeString(commandParts[2])}の正の約数を列挙してください。`, 'user', userName, 'user_icon.png', 'user');
        if (BigInt(commandParts[2]) > 9007199254740992n) {
            appendTypingIndicator();
            setTimeout(() => {
                removeTypingIndicator();
        ...
        const num = calculate(commandParts[2]);
