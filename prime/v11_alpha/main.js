...
// ワーカーAPIのインスタンス作成
const main = new MainProcessingSysyem();
...
async function processCommand(command) {
    ...
    try {
        if ((mainCommand === 'prime' || mainCommand === "p" || mainCommand === "prim" || mainCommand === '/prime' || mainCommand === "/p" || mainCommand === "/prim") && parts.length === 2) {
            ...
            await wait(1);
            let st = performance.now();
            const factors = await main.newPFD_v5(parseInt(parts[2]));
            let et = performance.now();
...
async function checkPrime() {
    const str = document.getElementById('prime-input').value.split("");
    const formula = document.getElementById('prime-input').value;
    const resultDiv = document.getElementById('prime-result');
    resultDiv.style.display = "";
    for (let i = 0; i < str.length; i++) {
        if (isNaN(parseInt(str[i])) && !str[i].match(/[\^\*×\/÷\-\+\(\)\!\ ]/) && isNaN(parseInt(toHankakuNumber(str[i])))) {
    ...
    const number = await main.evaluateFormula(formula);
